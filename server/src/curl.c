...
char tenantID[] = "796e2786bd9241c184aabad6105aeed8";
...
void Create_machine()
{
	...
	Get_commondpre(commondpre);
	strcpy(server, commondpre);
	strcat(server, "servers -d '{ \"server\":{ \"flavorRef\":\"6\",\"imageRef\":\"ee652bfb-5213-44d1-a046-752a55ead3aa\",\"metadata\":{\"My Server Name\":\"server\" }, \"name\":\"new-server-test4\",\"source_type\":\"image\", \"personality\":[{ \"contents\":\"\", \"path\":\"/etc/banner.txt\" }] }}'");
	fp = popen(server, "r");
	printf("%s\n", server);
	fgets(buffer, sizeof(buffer), fp);
	pclose(fp);
